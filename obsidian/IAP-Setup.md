...
1. **Play Console → Order management** (search the GPA number): the money truth, and where the **Refund** button lives. Refund-and-rebuy is the remedy of last resort; there is no server, so currency can't be pushed to a device remotely. **Always tick "remove entitlement" (refund AND revoke) when refunding a consumable.** A plain refund lets the account keep the unconsumed purchase, so Google keeps blocking rebuy with "already owned" indefinitely; revoking is what frees the SKU. (Learned the hard way during closed testing. A refunded-but-not-revoked purchase is still consumable by the app's recovery sync, which is the remaining unstick in that state.)
