    ...
    const update = () => {
      ...
      if (textRef.current) {
        if (ascended) {
          ...
          // of the same row (the rate readout). Showing 'current /
          // required' here was redundant and at high realms (Origin
          // Returning+) the numbers grew past what fits and got
          // cropped in the middle. Now we show ONLY the GOAL: the
          // required rate to clear the gate. Player compares against
          // the right-side current rate to see the gap.
          textRef.current.textContent = `${fmtRate(gate.required)}/s`;
        } else {
  ...
  useEffect(() => {
    ...
    const tick = (now) => {
      ...
      if (intensity > 0 && !hidden && layer && layer.clientWidth > 0 && layer.clientHeight > 0) {
        ...
        // Hidden (HomeScreen off-tab, vfx-disabled, or crystal lifted for
        // an evolution overlay). Reset the throttle so the first visible
        // frame after returning doesn't fire a spawn from the accumulated
        // delta - the steady-state cadence picks up cleanly instead.
        lastSpark = now;
      }
    ...
    const tick = () => {
      ...
      if (textRef.current) {
        if (ascended) {
          ...
          // rate is on the right side of the row.
          textRef.current.textContent = `${fmtRate(gate.required)}/s`;
        } else {
